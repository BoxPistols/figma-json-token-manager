  ...
  if (name === 'colors') {
    ...
    Object.values(groupedTokens).forEach((group) => {
      group.sort((a, b) => {
        ...
        return a.path[a.path.length - 1].localeCompare(
    ...
    <div>
      <div className={name === 'colors' ? 'space-y-8' : 'space-y-6'}>
        {Object.entries(groupedTokens).map(
          ([parentPath, groupTokens], groupIndex) => (
            <div
              key={parentPath}
              className={`
              space-y-2 
              ${
                name === 'colors' && groupIndex > 0
                      ...
                      {!isCompactMode && onTokenUpdate && (
                        <div className="mt-2 space-y-1">
                          <TokenEditor
                            ...
                            onStartEdit={() =>
                              setEditingTokenId(token.path.join('/'))
                            }
...
function TokenPreview({
  ...
  if (token.type === 'color') {
    ...
    const textColorForSample =
      isLightVariant && variant === 'lighter'
    ...
    const contrastWithSampleText = calculateContrastRatio(
      color,
      textColorForSample
    );
        ...
        >
          ...
          {!isCompact && (
            <div className="absolute inset-0 flex items-center justify-center">
              <span
                style={{ color: textColorForSample }}
          ...
          <div
            className={`absolute bottom-1 right-1 text-xs ${isCompact ? 'bg-black/80 text-white px-1 py-0' : 'bg-black/70 dark:bg-white/70 text-white dark:text-black px-1.5 py-0.5'} rounded ${contrastWithSampleText < 3 ? 'ring-1 ring-orange-400' : ''}`}
          >
            <span
              title={`現在のテーマ背景色との対比: ${contrastWithBackground.toFixed(2)}:1`}
            >
              {isCompact
  ...
  if (token.type === 'typography') {
    const typographyValue = token.value as Record<
      string,
          ...
          <span
            ...
            style={{
              ...
              letterSpacing: typographyValue.letterSpacing
                ? `${typographyValue.letterSpacing}px`
                : 'normal',
              color: textColor,
              textTransform: typographyValue.textTransform as TextTransform,
            }}
          >
            {sampleText}
          </span>
  ...
  if (token.type === 'spacing') {
    ...
    const displayValue =
      typeof token.value === 'string' ? token.value : `${token.value}px`;
    ...
    return (
      <div className="space-y-2">
        <div
          className={`w-full flex items-center justify-center rounded-md ${isCompact ? 'h-12' : 'h-16'} bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700`}
        >
          <div className="flex items-center space-x-2">
  ...
  if (token.type === 'size') {
    ...
    return (
      <div className="space-y-2">
        <div
          className={`w-full flex items-center justify-center rounded-md ${isCompact ? 'h-12' : 'h-20'} bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700`}
        >
          <div
  ...
  if (token.type === 'opacity') {
    ...
    return (
      <div className="space-y-2">
        <div
          className={`w-full flex items-center justify-center rounded-md ${isCompact ? 'h-12' : 'h-16'} bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 relative overflow-hidden`}
        >
          {/* チェッカーボード背景 */}
          <div
  ...
  if (token.type === 'borderRadius') {
    ...
    const displayRadius = Math.min(
      numericValue === 9999 ? 50 : numericValue,
      32
    ); // Cap at 32px for display
  ...
  if (token.type === 'borderColor') {
    ...
    return (
      <div className="space-y-2">
        <div
          className={`w-full flex items-center justify-center rounded-md ${containerHeight} bg-gray-50 dark:bg-gray-800`}
        >
          <div
  ...
  if (token.type === 'shadow') {
    ...
    return (
      <div className="space-y-2">
        <div
          className={`w-full flex items-center justify-center rounded-md ${containerHeight} bg-gray-100 dark:bg-gray-900 p-2`}
        >
          <div
  ...
  if (token.type === 'breakpoint') {
    ...
    return (
      <div className="space-y-2">
        <div
          className={`w-full flex items-center justify-center rounded-md ${containerHeight} bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700`}
        >
    ...
    return (
      <div className="space-y-2">
        ...
        {!isCompact && (
          <div className="text-center text-xs text-gray-500 dark:text-gray-400">
            {iconValue}{' '}
            {iconValue.includes('px')
              ? ''
              : token.path[token.path.length - 1]?.includes('stroke')
                ? 'stroke'
                : 'size'}
          </div>
        )}
  ...
  return (
    <div
      className={`w-full flex items-center justify-center bg-gray-50 dark:bg-gray-900 rounded-md ${isCompact ? 'h-12' : 'h-20'} border border-gray-200 dark:border-gray-700`}
    >
